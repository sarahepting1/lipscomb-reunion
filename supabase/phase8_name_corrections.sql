-- Lipscomb Family Reunion — Phase 8 name corrections
-- Run this once against your Supabase project (SQL Editor -> paste -> Run).
--
-- The 'correction' event type previously never touched the people table --
-- approving it just marked the submission reviewed, leaving Sarah to fix
-- anything (like a misspelled name) by hand in Supabase's Table Editor.
-- Now the admin UI has a real "Full name" field for corrections, and
-- approving actually applies it here when one is provided.
...
begin
  ...
  if v_sub.event_type = 'death' then
    ...
    -- Only the name is applied automatically. Anything else described in
    -- the free-text note is still Sarah's to action manually.
    if (v_payload->>'corrected_name') is not null and (v_payload->>'person_id') is not null then
      update people
      set name = v_payload->>'corrected_name',
          updated_at = now()
      where id = v_payload->>'person_id';
      if not found then
        raise exception 'Cannot approve: no matched person for this correction';
      end if;
    end if;
